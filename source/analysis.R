...
ggplot(state_shape) +
  geom_polygon(
    mapping = aes(x = long, y = lat, group = total_jail_pop),
    color = "white",
    size = .1
  ) + 
  coord_map()+
...
View(state_shape)
  ...
  #mutate(prop_in_jail = sum(total_jail_pop)/sum(total_pop)*100) %>%
  #group_by(state) %>%
  #summarise(combo_total_jail_pop = sum(total_jail_pop)) %>%
  #summarise(combo_total_pop = sum(total_pop)) %>%
  #summarise(combo_prop_in_jail = sum(prop_in_jail))
  
  select(state, prop_in_jail) #%>%
  state.abb[match("state",state.name)]
